...
const log = createLogger('API:issues/[id]/analysis');
...
export async function GET(
  ...
  try {
    ...
    if (authError || !user) {
      log.warn('Unauthenticated request to GET /api/issues/[id]/analysis');
    ...
    const analysis = await issueAnalysisService.getLatestAnalysis(user.id, id);

    log.info('GET /api/issues/[id]/analysis', {
      userId: user.id,
      issueId: id,
    ...
    log.error('GET /api/issues/[id]/analysis failed', { error: appErr.message });
