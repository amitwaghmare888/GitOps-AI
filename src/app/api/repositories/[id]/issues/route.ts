...
const log = createLogger('API:repositories/[id]/issues');
...
export async function GET(
  ...
  try {
    ...
    if (authError || !user) {
      log.warn('Unauthenticated request to GET /api/repositories/[id]/issues');
    ...
    const { data, count } = await issueService.getRepositoryIssues(user.id, repositoryId, parsed.data);
    log.info('GET /api/repositories/[id]/issues', { repositoryId, userId: user.id, count });

    return Response.json({ issues: data, total: count });
  } catch (err) {
    const appErr = toAppError(err);
    log.error('GET /api/repositories/[id]/issues failed', { error: appErr.message });
