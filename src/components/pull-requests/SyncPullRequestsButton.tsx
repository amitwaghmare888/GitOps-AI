...
interface SyncPullRequestsButtonProps {
  repositoryId: string;
  className?: string;
}

export function SyncPullRequestsButton({ repositoryId, className }: SyncPullRequestsButtonProps) {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      setState('success');
      setMessage(data.message ?? `Synced ${data.synced ?? 0} pull requests.`);
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      >
        ...
        {state === 'loading' ? 'Syncing…' : 'Sync PRs'}
      </button>
