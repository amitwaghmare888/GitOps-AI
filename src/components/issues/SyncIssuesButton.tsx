...
export function SyncIssuesButton({ repositoryId, className }: SyncIssuesButtonProps) {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/repositories/${repositoryId}/issues/sync`, { method: 'POST' });
      ...
      setState('success');
      setMessage(data.message ?? `Synced ${data.synced ?? 0} issues.`);
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      >
        ...
        {state === 'loading' ? 'Syncing…' : 'Sync Issues'}
      </button>
